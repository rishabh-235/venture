...
export default function FollowingCard(props) {
  ...
  useEffect(() => {
    if (userData && userData.following.includes(following._id)) {
      setIsFollowing(true);
      setText("Following");
    }
  }, [userData, following._id]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const resultAction = await dispatch(
        toggleFollowUser({ userId: following._id, isFollowing }),
      );
      ...
      <div className=" flex">
        ...
        <div className=" w-[27rem] flex flex-col justify-center ">
          <h3 className=" w-full text-start tracking-wider font-[600] py-[0.1rem] px-[0.1rem]">
            {following.firstname} {following.middlename} {following.lastname}
          </h3>
